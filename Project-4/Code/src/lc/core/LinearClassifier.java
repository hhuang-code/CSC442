...
abstract public class LinearClassifier {
	...
	public LinearClassifier(int inputs, double bound) {
		this.weights = new Random().doubles(inputs, -bound, bound).toArray();
	}
	...
	public void test(List<Example> examples, int nsteps, LearningRateSchedule schedule, double split) {
		// Shuffle all examples
		Collections.shuffle(examples);
		
		// Split all examples into training and testing parts
		List<Example> train_examples = examples.subList(0, (int)(examples.size() * (1 - split)));
		List<Example> test_examples = examples.subList((int)(examples.size() * (1 - split)), examples.size());
		
		Random random = new Random();
		int n = train_examples.size();
		for (int i=1; i <= nsteps; i++) {
			int j = random.nextInt(n);
			Example ex = train_examples.get(j);
			this.update(ex.inputs, ex.output, schedule.alpha(i));
			this.trainingReport(train_examples, i, nsteps);
			this.testingReport(test_examples, i, nsteps);
		}
	}
	
	public void test(List<Example> examples, int nsteps, double constant_alpha, double split) {
		test(examples, nsteps, new LearningRateSchedule() {
			public double alpha(int t) { return constant_alpha; }
		}, split);
	}
	
	protected void testingReport(List<Example> examples, int stepnum, int nsteps) {
		System.out.println(stepnum + "\t" + accuracy(examples));
	}
